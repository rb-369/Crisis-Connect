...
export default function VolunteerMock({ currentUser, onOpenAuthModal }) {
  ...
  const [sortCriteria, setSortCriteria] = useState('priority'); // 'priority' or 'distance'
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const data = await api.getRequests(null, filterActiveOnly, sortCriteria);
      setRequests(data);
    } catch (err) {
  ...
  const visibleRequests = requests
    ...
    .sort((a, b) => {
      if (sortCriteria === 'distance') {
        const distA = Math.hypot(a.lat - selectedDonorProfile.lat, a.lng - selectedDonorProfile.lng);
        const distB = Math.hypot(b.lat - selectedDonorProfile.lat, b.lng - selectedDonorProfile.lng);
        return distA - distB;
      }
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2 border-b border-slate-200">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-[#0F172A] flex items-center space-x-1.5">
                    <span>Priority Emergency Dispatch Feed</span>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold">
                      Live
                    </span>
                  </h4>
                  <span className="text-[11px] text-[#64748B] font-mono">
                    Showing {visibleRequests.length} genuine active incidents ({requests.length} total)
                  </span>
                </div>

                <div className="flex flex-wrap items-center gap-2">
                  <label className="flex items-center space-x-1.5 text-[11px] font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-lg cursor-pointer hover:bg-slate-200 transition">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span>Active Emergencies Only</span>
                  </label>

                  <select
                    value={sortCriteria}
                    onChange={(e) => setSortCriteria(e.target.value)}
                    className="text-[11px] font-bold bg-slate-100 border border-slate-300 text-slate-800 rounded-lg px-2 py-1 cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-500"
                  >
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                              ...
                              {r.priority_score !== undefined && (
                                <span className={`px-2 py-0.5 rounded-md text-[10px] font-black font-mono shadow-2xs ${
                                  r.priority_score >= 70 ? 'bg-red-600 text-white' :
                                  r.priority_score >= 45 ? 'bg-amber-500 text-white' :
                              ...
                              {(r.zone_confirmed || r.ml_status) && (
                                <span className="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 font-extrabold text-[10px] flex items-center space-x-1">
                                  <Sparkles className="w-3 h-3 text-emerald-600" />
                                  <span>⚡ Confirmed Disaster Zone</span>
                                </span>
                              )}
